...
freeproc(struct proc *p)
{
  ...
  {
    free_proc_kernel_pagetable(p->kernel_pagetable);
  }
...
int growproc(int n)
{
  ...
  if (n > 0)
  {
    ...
    user2kernel_vmcopy(p->pagetable, p->kernel_pagetable, sz - n, sz);
  }
  else if (n < 0)
  {
...
int fork(void)
{
  ...
  np->sz = p->sz;
  user2kernel_vmcopy(np->pagetable, np->kernel_pagetable, 0, np->sz);
  np->parent = p;
